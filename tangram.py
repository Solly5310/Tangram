...
#line 119 Degrees checker was revised to provide list of float numbers, then summed the list by rounding to the nearest whole number, then conversionto an int
#line 214 & 218 when calling upon the dict in shape_checker function, needed to condition that if the value of a dictionary key is an int, that it doest calculate len, but rather int itself
#line 69 & 71, changed variable when sorting lines in XML files to include has 'M' in it to avoid any empty lines with or without tabs
#line 125 added length check which checks the length of each shape points equals the comparison shape
